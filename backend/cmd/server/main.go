...
func main() {
	...
	// Public Invite Preview (Rate limit: 30 req/min por IP)
	r.With(httprate.LimitByIP(30, time.Minute)).Get("/api/invites/{code}", inviteHandler.GetInvite)
	...
	r.Group(func(r chi.Router) {
		...
		joinGuildLimiter := ratelimit.NewUserRateLimiter(10, 10.0/60.0, "Você está entrando em servidores muito rápido. Aguarde um momento.")
		callInviteLimiter := ratelimit.NewUserRateLimiter(6, 6.0/60.0, "Você está iniciando chamadas muito rápido. Aguarde um momento.")
		voiceJoinLimiter := ratelimit.NewUserRateLimiter(15, 15.0/60.0, "Você está alternando de canais de voz muito rápido. Aguarde um instante.")
		reactionLimiter := ratelimit.NewUserRateLimiter(30, 30.0/60.0, "Você está reagindo muito rápido. Aguarde um instante.")
		...
		r.With(sensitiveActionLimiter.Middleware).Post("/api/auth/2fa/enable", authHandler.Enable2FA)
		...
		r.With(sensitiveActionLimiter.Middleware).Post("/api/auth/change-phone", authHandler.ChangePhone)
		r.With(profileUpdateLimiter.Middleware).Patch("/api/users/@me", userHandler.UpdateProfile)
		...
		r.Get("/api/guilds/{id}/invites", inviteHandler.ListGuildInvites)
		r.Post("/api/guilds/{id}/invites", inviteHandler.CreateInvite)
		r.Delete("/api/guilds/{id}/invites/{code}", inviteHandler.DeleteInvite)
		r.Post("/api/guilds/{id}/transfer-ownership", guildHandler.TransferOwnership)
		r.Get("/api/guilds/{id}/emojis", guildHandler.ListEmojis)
		r.With(emojiLimiter.Middleware).Post("/api/guilds/{id}/emojis", guildHandler.CreateEmoji)
		...
		r.With(callInviteLimiter.Middleware).Post("/api/dms/{roomID}/call/invite", dmHandler.InviteCall)
		r.With(callInviteLimiter.Middleware).Post("/api/dm/rooms/{roomID}/call/invite", dmHandler.InviteCall)
