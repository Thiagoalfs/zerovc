...
func Log(ctx context.Context, db *database.DB, hub *gateway.Hub, guildID, actorID uuid.UUID, actionType string, targetID *uuid.UUID, details map[string]any) {
	...
	_ = db.Pool.QueryRow(ctx, SELECT id, username, display_name, avatar_url, banner_url, bio, status, custom_status FROM users WHERE id = , actorID).Scan(
	...
	if targetID != nil {
		...
		err = db.Pool.QueryRow(ctx, SELECT id, username, display_name, avatar_url, banner_url, bio, status, custom_status FROM users WHERE id = , *targetID).Scan(
